...
local instance =
  ...
  function(params)
    ...
    if suffix == nill then
      suffix = UI.editor.currentClassSuffix
    end

    local controller
    local mod = UI.editor:getClassModule(class or "properties")
    if mod == nil then
      print("mod not found for ", class)
      controller = require(kwikGlobal.ROOT.."editor.controller.index")
    else
      controller = mod.controller -- each tool.contoller can overide render/save. So page tools of audio, group, timer should use own render/save
    end
    if props.shapedWith then
      ...
      for k, v in pairs(controller:useClassEditorProps(UI) or {} ) do
