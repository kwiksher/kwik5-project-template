...
local nanostores = require("nanostores.index")
  ...
  if app.editorContextInit == nil then
    ...
    for i = 1, #self.commands do
      app.context:mapCommand(
        ...
        kwikGlobal.ROOT.."editor.controller.selector." .. self.commands[i].name
      )
    end
...
function M:runTest(UI)
  timer.performWithDelay(
    500,
    function()
      require(kwikGlobal.ROOT.."test.index").run {
        selectors = selectors,
        UI = UI,
        bookTable = bookTable,
        pageTable = pageTable,
...
function M:resetGotoLastSelection()
  -- print("resetGotoLastSelection")
  local path = system.pathForFile("kwik.json", system.ApplicationSupportDirectory)
  if path then
    local result, reason = os.remove(path)
    if result then
      print("Last selection file removed successfully")
    else
      print("Failed to remove file:", reason)
    end
  end
end
...
function M:didShow(UI)
  ...
  if kwikGlobal.gotoLastBook == true then  -- comes from props.gotoLastBook
    self:gotoLastSelection() -- self.lastSelection
    kwikGlobal.gotoLastBook = nil
  elseif kwikGlobal.gotoLastBook == false then -- comes from props.gotoLastBook
    self:resetGotoLastSelection()
    kwikGlobal.gotoLastBook = nil
  end
