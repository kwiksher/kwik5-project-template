...
function M.createPage(book, _index, _page, _root, _weight)
  ...
  local tmplt = kwikGlobal.RESOUECES_PATH.."template/index.lua"
  local pages = {}
...
function M.copyPage(book, page, newName, _dst)
  ...
  --
  local path = system.pathForFile(kwikGlobal.RESOUECES_PATH.."template/index.lua", system.ResourceDirectory)
  local file, errorString = io.open(path, "r")
  if not file then
    print("ERROR: " .. errorString)
  else
