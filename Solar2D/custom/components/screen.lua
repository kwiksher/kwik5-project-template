...
local function createUI()
    ...
    local tooltip = display.newText( group, "screen bounds (red)", screen.safe.minX + 20, 120, native.systemFontBold, 12 )
    ...
    local safe_tooltip = display.newText( group, "safe screen bounds (green)", screen.safe.minX + 20, tooltip.y + tooltip.height + 4, native.systemFontBold, 12 )
    ...
    local disclaimer = display.newText( group, "NB! The borders extend beyond the screens to make them easier to see. Their center points are on the edges.", screen.safe.minX + 20, safe_tooltip.y + safe_tooltip.height + 12, 400, 0, native.systemFont, 10 )
