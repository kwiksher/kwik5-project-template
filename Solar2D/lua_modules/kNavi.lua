...
M.new = function(_params, naviListener)
  ...
  for i = 1,#sceneList do
    ...
    if  path then
      local obj =  display.newImage(app.props.thumbDir..scene..".png", 0, 0)
      if obj.width > obj.height then
        menuTab[i] = display.newImageRect(app.props.thumbDir..scene..".png", UI.props.systemDir, thumbW, thumbH)
      else
        menuTab[i] = display.newImageRect(app.props.thumbDir..scene..".png", UI.props.systemDir, thumbH, thumbW)
      end
      obj:removeSelf()
      ...
      menuTab[i] = display.newText(options)
      if currentPage ~= scene then
        menuTab[i].scene = scene
        menuTab[i]:setFillColor(0, 0, 0, 1)
      else
        menuTab[i]:setFillColor(0, 0, 0, 0.5)
      end
    end
