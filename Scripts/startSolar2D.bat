...
if /i not "%SCALE_ARG%"=="1x" if /i not "%SCALE_ARG%"=="1" if /i not "%SCALE_ARG%"=="2x" if /i not "%SCALE_ARG%"=="2" (
...
if "%SINGLETON%"=="1" (
    ...
    for /f "tokens=2" %%i in ('tasklist /fi "imagename eq Corona Simulator.exe" /fo csv /nh') do (
        taskkill /pid %%~i /f >nul 2>&1
    )
    for /f "tokens=2" %%i in ('tasklist /fi "imagename eq Corona.Console.exe" /fo csv /nh') do (
        taskkill /pid %%~i /f >nul 2>&1
    )
)
...
if not defined filePath set "filePath=%~dp0\Solar2D\main.lua"
